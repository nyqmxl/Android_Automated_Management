...
uri = "http://192.168.0.106:8000"
if (isfile("文件/梦云运行框架.json")):
    with open("文件/梦云运行框架.json", "r", encoding="UTF-8") as fp:
...
class 页面:
    def 文档帮助():
        if (isfile("文件/运行框架帮助文档.md")):
            with open("文件/运行框架帮助文档.md", "r", encoding="UTF-8") as fp:
                markdown(fp.read())
        else:
            markdown("## 帮助文档")
            markdown("---")
            markdown("找不到文档说明，请把文档放入 “文件/运行框架帮助文档.md” 中在试试！")

    def 设备信息():
        markdown("## 设备信息")
        data_return = None
        if (button("获取设备状态")):
            data_return = get(F"{uri}/api/status").json()
        if (data_return):
            col_return = list(columns(6))
            for f1 in range(len(data_return)):
                if (not "名称" in data_return[f1].keys()):
                    data_return[f1]["名称"] = data_return[f1]["_id"]
                with col_return[f1 % 6].popover(data_return[f1]["_id"]):
                    f1d = data_return[f1]["数据"][0]["设备状态"]
                    markdown(F'### 设备：{f1d["设备名称"]} 的详细情况')
                    markdown("---")
                    f1d["设备充电"] = "是" if f1d["设备充电"] else "否"
                    f1d["屏幕常量"] = "是" if f1d["屏幕常量"] else "否"
                    f1d["物理内存"] = "，".join(f1d["物理内存"])
                    f1d["交换内存"] = "，".join(f1d["交换内存"])
                    f1d["存储信息"] = "，".join(f1d["存储信息"])
                    f1d["设备地址"] = "，".join(
                        [f1d if type(f1d) == str else "\t".join(f1d.values()) for f1d in f1d["设备地址"]])
                    table({"数据": f1d})
                    # write(f1)
...
def 函数解析(pointer_meun):  # 包含菜单布局
    ...
    pointer_meun = [页面.文档帮助, 页面.设备信息, 页面.队列任务, 页面.任务日志, 页面.文件管理]
